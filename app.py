...
def get_search(query_type: QUERY_TYPE) -> str:
    ...
    table_html = get_html_table(page_data, query_type)
    visualization = get_html_visualization(result, query_type)
    pagination_html = get_pagination_html(current_page=page, total_pages=total_pages, query_type=query_type)
    
    return  pagination_html + table_html +  visualization
