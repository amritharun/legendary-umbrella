...
def get_html_table(result: tuple, query_type: QUERY_TYPE) -> str:
    ...
    original_value = None
    if len(result) > 0 and check_same_cik(result):
...
def get_value_row(item: dict, original_value: float) -> str:
    ...
    if value is None:
        ...
        return f"""<td><p>{escape(locale.currency(value, grouping=True))}</p></td>"""
    elif value < original_value:
        return f"""<td><p style="color: red;">{escape(locale.currency(value, grouping=True))}</p></td>"""
    elif value > original_value:
        return f"""<td><p style="color: green;">{escape(locale.currency(value, grouping=True))}</p></td>"""
    else:
        return f"""<td><p>{escape(locale.currency(value, grouping=True))}</p></td>"""
